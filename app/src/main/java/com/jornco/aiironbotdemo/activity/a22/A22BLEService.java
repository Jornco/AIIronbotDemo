...
class A22BLEService extends BluetoothGattCallback implements MultiIronbotWriterCallback.OnSendListener{
    ...
    private List<A22BLEMotor> mMotorList;
    ...
    public A22BLEService(IronbotInfo info) {
        mInfo = info;
    ...
    public A22BLESession getSession(Context context) {
        String address = mInfo.getAddress();
        mGatt = getConnection(context, address);
        if (mGatt == null) {
            return null;
        }
        return new A22BLESession(mGatt, this);
    }
