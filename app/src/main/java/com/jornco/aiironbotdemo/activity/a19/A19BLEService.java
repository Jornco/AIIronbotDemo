...
public class A19BLEService extends BluetoothGattCallback implements MultiIronbotWriterCallback.OnSendListener{
    ...
    public A19BLEService(IronbotInfo info) {
        mInfo = info;
    ...
    public A19BLESession getSession(Context context) {
        String address = mInfo.getAddress();
        mGatt = getConnection(context, address);
        if (mGatt == null) {
            return null;
        }
        return new A19BLESession(mGatt, this);
    }
